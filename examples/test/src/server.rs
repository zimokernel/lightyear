...
pub(crate) fn handle_connections(
    ...
    mut server_net: ResMut<ConnectionManager>,
) {
    for connection in connections.read() {
        ...
        info!(
            "[Server] Create entity {:?} for client {:?}",
            entity.id(),
            client_id
        );
        ...
        info!("Large message sent len:{} to server:  {:?}... ",message.data.len(), &message.data[..10]);
    }
}
...
pub(crate) fn send_message(
    ...
    if input.is_some_and(|input| input.pressed(KeyCode::KeyM)) {
        let message = Message1(5);
        info!("[Server] Send message: {:?}", message);
        server
            .send_message_to_target::<Channel1, Message1>(&Message1(5), NetworkTarget::All)
            .unwrap_or_else(|e| {
                error!("Failed to send message: {:?}", e);
...
pub(crate) fn handle_disconnections(
    ...
    for disconnection in disconnections.read() {
        debug!("[Server] Client {:?} disconnected", disconnection.client_id);
...
pub(crate) fn movement(
    ...
    for input in input_reader.read() {
        ...
        if let Some(input) = input.input() {
            trace!(
                "[Server] Receiving input: {:?} from client: {:?} on tick: {:?}",
                input,
                client_id,
                tick_manager.tick()
            );
