...
fn recieve_messages(
	...
	for message in large_messages.read() {
		info!("[Client] Rx Large message recieved:len{} {:?}...",message.message.data.len(), &message.message.data[..10]);
	}
}
...
fn on_disconnect(mut commands: Commands, player_entities: Query<Entity, With<PlayerId>>) {
    for entity in player_entities.iter() {
        commands.entity(entity).despawn_recursive();
    }
    info!("client on disconnect");
}

fn start_connect(mut commands: Commands) {
    info!("client start connect");
